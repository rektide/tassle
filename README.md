...
| Capability | Status |
|---|---|
| Rust clap CLI (`crates/tass-cli`) | ✅ primary |
| Root Rust workspace | ✅ |
| Lexicon schema (`crates/tass-lex-schema`) + sample (`crates/tass-lex-sample`) crates | ✅ |
| Rust Jacquard public XRPC record access (`repo list`) | ✅ |
| Rust generated builders for tass lexicons | ✅ |
...
```bash
cargo build -p tass-cli
```

The active CLI is the Rust binary in `crates/tass-cli`. Run it from the repository root with `cargo run -p tass-cli -- ...`.
...
cargo run -p tass-cli -- repo list actor.rpg.stats --repo jauntywk.bsky.social

# Read normalized Mage stats from the active profile
cargo run -p tass-cli -- mage stats

# List every actor.rpg.stats rkey for the active profile
cargo run -p tass-cli -- repo list actor.rpg.stats

# Generate a Node record as validated JSON
cargo run -p tass-cli -- generate node "Crystal Spring" -r 3 -R dynamic -t "a smooth river-stone"

# Regenerate example records via xtask
cargo xtask samples
```
...
Rust profile defaults are stored as TOML fragments under `${XDG_CONFIG_HOME:-~/.config}/tass/config.toml.d/`. `auth login` currently resolves and stores the profile DID/PDS only; it does not perform OAuth yet.
...
tass repo list actor.rpg.stats

# Same collection from an explicit actor
tass repo list actor.rpg.stats --repo jauntywk.bsky.social

# Future: filter by record envelope or value fields
tass repo list actor.rpg.stats --where 'rkey == "mage"'

# Future: pick result positions after filtering
tass repo list actor.rpg.stats --pick '1,3,6-7,-2--1'
```
...
tassle/
├── crates/
│   ├── tass-lex-schema/           # canonical lexicon JSON (source-of-truth)
│   ├── tass-lex-sample/           # example record fixtures (regenerated by xtask)
│   ├── tass-cli/                # primary clap/Jacquard CLI
│   ├── tass-lex/                  # generated Rust lexicon types/builders (at.telluri.*)
│   ├── tass-codegen/            # Jacquard codegen wrapper
│   └── tass-validate/           # schema validation helper
├── xtask/                         # workspace tasks: codegen + sample regen
...
`crates/tass-lex-sample/samples/` contains example records generated from the builders via `cargo xtask samples`. Fixed `createdAt` for stable diffs; placeholder DID (`did:plc:samplesamplesamplesample`) since the canonical publisher isn't decided. Four examples cover the full lifecycle: node → tassilize → enervate, plus a meditate. The `tass-lex-sample` crate embeds these as fixtures; the lexicon schemas live separately in `tass-lex-schema`.

# Development

```bash
cargo check -p tass-cli
cargo run -p tass-cli -- repo list actor.rpg.stats --repo jauntywk.bsky.social

# Regenerate derived artifacts:
cargo xtask codegen    # tass-lex-schema/lexicons -> tass-lex/src
cargo xtask samples    # generated builders -> tass-lex-sample/samples
...
Rust toolchain: root Cargo workspace, clap v4 derive, Jacquard for atproto/XRPC, `tass-lex-schema` lexicon JSON and `tass-lex-sample` fixtures, generated `tass-lex` builders, `miette` for diagnostics, `tokio` for async runtime, `cargo xtask` for codegen/sample regen.
