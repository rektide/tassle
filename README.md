...
cargo xtask codegen    # tass-lex-schema/lexicons -> tassle-lexicons/src
...
Rust toolchain: root Cargo workspace, clap v4 derive, Jacquard for atproto/XRPC, `tass-lex-schema` lexicon JSON and `tass-lex-sample` fixtures, generated `tassle-lexicons` builders, `miette` for diagnostics, `tokio` for async runtime, `cargo xtask` for codegen/sample regen.
