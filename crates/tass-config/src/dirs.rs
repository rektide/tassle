...
//!
//! [`appname`] is the leaf directory under each base: `TASS_APPNAME` if set,
//! else `"tass"`. Each root additionally honours a full-path override env
//! (`TASS_CONFIG_DIR` / `TASS_DATA_DIR` / `TASS_STATE_DIR` /
//! `TASS_CACHE_DIR`); when set it is used verbatim (the appname is *not*
...
/// Built-in application directory name; overridden by `TASS_APPNAME`.
const DEFAULT_APPNAME: &str = "tass";
...
/// [`Overrides::appname`] > `TASS_APPNAME` > the built-in `"tass"`
...
/// [`Overrides::config_dir`] > `TASS_CONFIG_DIR` > `XDG_CONFIG_HOME` > `$HOME`.
pub fn config_dir() -> miette::Result<PathBuf> {
    resolve_from(
        flag_or_env(overrides().config_dir.as_deref(), "TASS_CONFIG_DIR"),
...
/// `TASS_STATE_DIR` > `XDG_STATE_HOME` > `$HOME`.
pub fn state_dir() -> miette::Result<PathBuf> {
    resolve_from(
        flag_or_env(overrides().state_dir.as_deref(), "TASS_STATE_DIR"),
...
mod tests {
    ...
    fn override_wins_verbatim_without_appname() {
        let got = resolve_from(os("/srv/tass-cfg"), os("/xdg"), Some(Path::new("/home/u")), &[".config"], "tass").unwrap();
        assert_eq!(got, PathBuf::from("/srv/tass-cfg"));
    }

    #[test]
    fn xdg_base_gets_appname_appended() {
        let got = resolve_from(None, os("/xdg/config"), Some(Path::new("/home/u")), &[".config"], "tass").unwrap();
        assert_eq!(got, PathBuf::from("/xdg/config/tass"));
    }

    #[test]
    fn relative_xdg_is_ignored_falls_back_to_home() {
        let got = resolve_from(None, os("relative/nope"), Some(Path::new("/home/u")), &[".local", "state"], "tass").unwrap();
        assert_eq!(got, PathBuf::from("/home/u/.local/state/tass"));
    }
    ...
    fn missing_home_without_bases_errors() {
        assert!(resolve_from(None, None, None, &[".config"], "tass").is_err());
    }
}
