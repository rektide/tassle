...
//!
//! Model (ported from tass-cli's `config.rs`): a base `config.toml` carries
...
//! `config.toml.d/<did>.toml` fragment-per-DID bridge (tass-cli's
...
/// How a command chooses a credential to read with. Parsed from the top-level
/// `auth` config key (profile-resolved like everything else); absent means the
/// default, [`CredentialSelector::ActiveIfAvailable`].
///
/// This governs *reads* (writes always need a concrete authed session). Pure —
/// it names an intent; resolving it to a live session is the `auth-store`
/// layer's job (see `tass_config::auth`). Keeping it here, ungated, lets the
/// default build parse and reason about the policy without pulling jacquard.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub enum CredentialSelector {
    /// `@none` — read unauthenticated (a public `getRecord`/`listRecords`).
    None,
    /// `@active` — the store's active-account session. An error if no active
    /// account is set or its session cannot be resumed.
    Active,
    /// `@active-if-available` (the default) — the active-account session if one
    /// is present and resumable, else fall back to unauthenticated. Never errors
    /// on a missing session, so the default build reads public data with no
    /// login and no auth deps engaged.
    #[default]
    ActiveIfAvailable,
    /// A specific identity — a handle or DID resolved **directly** against the
    /// session store (`first_by_handle` / `first_by_did`), bypassing profiles.
    Named(String),
}

impl std::str::FromStr for CredentialSelector {
    type Err = String;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let s = s.trim();
        match s {
            "@none" => Ok(CredentialSelector::None),
            "@active" => Ok(CredentialSelector::Active),
            "@active-if-available" => Ok(CredentialSelector::ActiveIfAvailable),
            "" => Ok(CredentialSelector::default()),
            _ if s.starts_with('@') => Err(format!(
                "unknown auth selector `{s}` (expected @none, @active, @active-if-available, or a handle/DID)"
            )),
            _ => Ok(CredentialSelector::Named(s.to_string())),
        }
    }
}

impl std::fmt::Display for CredentialSelector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CredentialSelector::None => f.write_str("@none"),
            CredentialSelector::Active => f.write_str("@active"),
            CredentialSelector::ActiveIfAvailable => f.write_str("@active-if-available"),
            CredentialSelector::Named(name) => f.write_str(name),
        }
    }
}

/// The active profile's read-credential policy from the `auth` config key,
/// defaulting to [`CredentialSelector::ActiveIfAvailable`] when absent or empty.
pub fn auth_selector(figment: &Figment) -> miette::Result<CredentialSelector> {
    let Ok(value) = figment.find_value("auth") else {
        return Ok(CredentialSelector::default());
    };
    let raw = value
        .into_string()
        .ok_or_else(|| miette::miette!("config key `auth` must be a string"))?;
    raw.parse().map_err(|e| miette::miette!("{e}"))
}
...
/// or `TASS_PROFILE`), it is injected as the `profile` key.
...
/// The active profile name from `TASS_PROFILE`, if set.
pub fn profile_from_env() -> Option<String> {
    std::env::var("TASS_PROFILE")
        .ok()
...
pub fn active_login(figment: &Figment) -> miette::Result<Login> {
    figment
        .extract::<Login>()
        .map_err(|e| miette::miette!("failed to extract tass login: {e}"))
}
...
mod tests {
    ...
    #[test]
    fn credential_selector_parses_sigils() {
        use std::str::FromStr;
        assert_eq!(CredentialSelector::from_str("@none").unwrap(), CredentialSelector::None);
        assert_eq!(CredentialSelector::from_str("@active").unwrap(), CredentialSelector::Active);
        assert_eq!(
            CredentialSelector::from_str("@active-if-available").unwrap(),
            CredentialSelector::ActiveIfAvailable
        );
        // Whitespace tolerated.
        assert_eq!(CredentialSelector::from_str("  @active ").unwrap(), CredentialSelector::Active);
        // Empty / absent => default.
        assert_eq!(CredentialSelector::from_str("").unwrap(), CredentialSelector::ActiveIfAvailable);
    }

    #[test]
    fn credential_selector_named_and_unknown_sigil() {
        use std::str::FromStr;
        assert_eq!(
            CredentialSelector::from_str("alice.bsky.social").unwrap(),
            CredentialSelector::Named("alice.bsky.social".into())
        );
        assert_eq!(
            CredentialSelector::from_str("did:plc:abc").unwrap(),
            CredentialSelector::Named("did:plc:abc".into())
        );
        // An unrecognized @sigil is rejected, not treated as a name.
        assert!(CredentialSelector::from_str("@bogus").is_err());
    }

    #[test]
    fn credential_selector_roundtrips_through_display() {
        use std::str::FromStr;
        for s in ["@none", "@active", "@active-if-available", "alice.test"] {
            let parsed = CredentialSelector::from_str(s).unwrap();
            assert_eq!(CredentialSelector::from_str(&parsed.to_string()).unwrap(), parsed);
        }
    }

    #[test]
    fn auth_selector_defaults_and_reads() {
        // Absent => default.
        assert_eq!(auth_selector(&Figment::new()).unwrap(), CredentialSelector::ActiveIfAvailable);
        // Present => parsed.
        let fig = Figment::from(Serialized::default("auth", "@none"));
        assert_eq!(auth_selector(&fig).unwrap(), CredentialSelector::None);
        let fig = Figment::from(Serialized::default("auth", "did:plc:xyz"));
        assert_eq!(auth_selector(&fig).unwrap(), CredentialSelector::Named("did:plc:xyz".into()));
    }
