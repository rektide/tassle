...
pub struct Master<S: BosStr = DefaultStr> {
    ...
    pub player: Did<S>,
    ///What portions of stats are validated. 'none' = inherent trust (always valid), 'custom' = selected fields only, 'full' = all fields must match. Defaults to 'full' if omitted.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl<S: BosStr> LexiconSchema for Master<S> {
    ...
    fn validate(&self) -> Result<(), ConstraintError> {
        ...
        if let Some(ref value) = self.snapshot_scope {
            #[allow(unused_comparisons)]
            if <str>::len(value.as_ref()) > 20usize {
                return Err(ConstraintError::MaxLength {
                    path: ValidationPath::from_field("snapshot_scope"),
                    max: 20usize,
                    actual: <str>::len(value.as_ref()),
                });
            }
        }
        if let Some(ref value) = self.sprite_cid {
            #[allow(unused_comparisons)]
            if <str>::len(value.as_ref()) > 200usize {
                return Err(ConstraintError::MaxLength {
                    path: ValidationPath::from_field("sprite_cid"),
                    max: 200usize,
                    actual: <str>::len(value.as_ref()),
                });
            }
