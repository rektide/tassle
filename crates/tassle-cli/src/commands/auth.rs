...
async fn session_status(
    ...
    // Store access lives in tassle-config; the CLI only decodes expiry.
    let access_jwt = match tassle_config::stored_access_jwt(store_path, did, session_id).await {
        Ok(Some(jwt)) => jwt,
...
/// `tassle_config::AuthedClient::login`, then persist the non-secret profile
...
async fn login_real(
    ...
    // App password: --password > TASSLE_PASSWORD > interactive prompt. Reading
    ...
    // The one authed-client construction path (tassle-config): resolve store,
